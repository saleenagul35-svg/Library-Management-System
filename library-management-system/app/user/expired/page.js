...
function EmptyState() {
    return (
        <div className="bg-[#fffff3] rounded-xl border border-[#d9d4c2] py-16 flex flex-col items-center text-center px-6">
            ...
            <p className="text-xs text-[#b0a98f] leading-relaxed">No expired requests.<br />Expired requests will appear here.</p>
        </div>
    );
}
    ...
    return (
        <div className="hidden md:block bg-[#fffff3] rounded-xl border border-[#d9d4c2] overflow-hidden">
            <div className="grid bg-[#FCF5E1] border-b border-[#d9d4c2] px-5 py-2.5"
                ...
                {["Book", "Author", "ISBN", "Expired", "Status"].map((h, i) => (
...
function DesktopRow({ r, index, isLast }) {
    return (
        ...
        >
            ...
            <div>
                <p className="text-[12px] font-medium text-[#2e2c1e]">{formatDate(r.expireDate)}</p>
            </div>
            ...
            {requests.map((r, idx) => (
                <div key={r.userId.id} className="bg-[#fffff3] rounded-xl border border-[#d9d4c2] p-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[11px] text-[#9a9280]">{formatDate(r.expireDate)}</span>
                ...
                <div className="flex items-end justify-between mb-6">
                    <div>
                        <h1 className="font-serif text-[18px] font-medium text-[#2e2c1e] tracking-tight">
                            Missed Collection Window
                        </h1>
                        <p className="text-xs text-[#9a9280] mt-1">Pickup window closed. Please submit a new request if you still wish to borrow this book.</p>

                    </div>
                    <span className="text-[11px] font-medium text-[#fffff3] bg-[#e6a832] px-3.5 py-1.5 rounded-full">
                        {requests.length} expired
                    </span>
                </div>

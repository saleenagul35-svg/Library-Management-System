...
function EmptyState() {
  return (
    <div className="bg-[#fffff3] rounded-xl border border-[#d9d4c2] py-16 flex flex-col items-center text-center px-6">
      ...
      <p className="text-xs text-[#b0a98f] leading-relaxed">No approved requests.<br />Approved requests will appear here.</p>
    </div>
  );
}
  ...
  return (
    <div className="hidden md:block bg-[#fffff3] rounded-xl border border-[#d9d4c2] overflow-hidden">
      <div className="grid bg-[#FCF5E1] border-b border-[#d9d4c2] px-5 py-2.5"
        ...
        {["Book", "Author", "ISBN", "Approved", "Status"].map((h, i) => (
...
function DesktopRow({ r, index, isLast }) {
  return (
    ...
    >
      ...
      <div>
        <p className="text-[12px] font-medium text-[#2e2c1e]">{formatDate(r.approvedDate)}</p>
      </div>
      ...
      {requests.map((r, idx) => (
        <div key={r.userId.id} className="bg-[#fffff3] rounded-xl border border-[#d9d4c2] p-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[11px] text-[#9a9280]">{formatDate(r.approvedDate)}</span>
...
export default function ApprovedRequests() {
  ...
  const fetchingAPIs = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/UserApprovedRequestData", {
        method: "GET",
        headers: {
